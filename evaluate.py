...
import subprocess
import argparse
from flashrender_utils.model_utils import make_camera_trajectory
from omegaconf import OmegaConf
from pathlib import Path
from flashrender_utils.eval_utils import eval_rot, eval_trans

def load_featup():
    """Load the FeatUp DINO upsampler used by Dyn-MEt3R (reusable across videos)."""
    upsampler = torch.hub.load('mhamilton723/FeatUp', 'dino16', use_norm=True).eval()
    return upsampler.to('cuda')


def load_met3r():
    """Load the MEt3R metric (reusable across videos)."""
    return MEt3R(
...
##############################################################################################################
@torch.no_grad()
def calcuate_dyn_met3r(dir_name, video_name, size=224, upsampler=None):
    ...
    owns_upsampler = upsampler is None
    if owns_upsampler:
        upsampler = load_featup()
...
# Initialize MEt3R
def calcuate_MEt3R(dir_name, video_name, metric=None):
    if metric is None:
        metric = load_met3r()
...
def evaluate_one(data_path, upsampler=None, met3r=None):
    """Evaluate a single generated video and write its .txt metrics file.

    Pass pre-loaded `upsampler` (FeatUp) and `met3r` (MEt3R) to avoid reloading
    them per call; if None they are loaded internally (standalone usage)."""
    dir_name = Path(data_path).parents[0]
    video_name = os.path.splitext(os.path.basename(os.path.normpath(data_path)))[0]
    cam_type = video_name.split('_cam')[-1]
    ...
    output_path = Path(dir_name) / f'{video_name}.txt'

    try:
        RotErr, TransErr = calculate_pose_error(dir_name, video_name, cfg)
    except Exception as e:
        print(f"[Pose Error] {dir_name}/{video_name}: {e}")
        RotErr, TransErr = None, None

    try:
        dyn_met3r = calcuate_dyn_met3r(dir_name, video_name, upsampler=upsampler)
    except Exception as e:
        print(f"[DYN-MET3R Error] {dir_name}/{video_name}: {e}")
        dyn_met3r = None

    try:
        pairwise_met3r = calcuate_MEt3R(dir_name, video_name, metric=met3r)
    except Exception as e:
        print(f"[MET3R Error] {dir_name}/{video_name}: {e}")
        pairwise_met3r = None
    ...
    # clean up tmp artifacts (guard each in case that stage failed)
    for path, remover in [
        (os.path.join(dir_name, video_name), shutil.rmtree),           # ViPE folder
        (os.path.join(dir_name, 'resized_' + video_name), shutil.rmtree),  # tmp folder
        (os.path.join(dir_name, f'resized_{video_name}.mp4'), os.remove),  # tmp video
    ]:
