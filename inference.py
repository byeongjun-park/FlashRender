...
import hydra
import os
from flashrender_utils.model_utils import adjust_to_FlashRender, save_video, make_camera_trajectory
import subprocess
import glob
...
def load_video_with_captions(cfg, save_dir):
    ...
    if cfg.eval.video_path.endswith('.mp4'):
        ...
        if eval_frames > 81 and cfg.eval.clamp_frames:
            eval_frames = 81
    ...
    rel_poses = np.linalg.inv(cond_c2ws) @ target_c2ws
    rel_poses = rearrange(torch.as_tensor(rel_poses[:, :3], dtype=intrinsics.dtype), 'b c d -> b (c d)')
    ...
    data = {
        ...
        'rel_poses': rel_poses.unsqueeze(0)
    }
...
def main(cfg: DictConfig):
    ...
    save_path = f"{cfg.eval.output_dir}/{video_name}"
    os.makedirs(save_path, exist_ok=True)

    data, num_frames, fps = load_video_with_captions(cfg, save_path)
    ...
    setattr(pipe, 'dit', adjust_to_FlashRender(getattr(pipe, 'dit'), cfg, training=False))

    state_dict = load_state_dict(cfg.eval.ckpt_path, torch_dtype=pipe.torch_dtype, device=pipe.device)

    remove_keys = [k for k in state_dict.keys() if k.startswith('alignment_loss.')] # this layer is used only for training
    ...
    video = pipe(
        ...
        rel_poses=data['rel_poses'].to('cuda'),
    ...
    m = re.search(r"(?:(meanflow|onpolicy)_)?epoch_(\d+)\.safetensors$", os.path.basename(cfg.eval.ckpt_path))
    prefix = f"{m.group(1)}_" if m.group(1) else ""
    save_video(video, f"{save_path}/{prefix}epoch{m.group(2)}_{cfg.eval.num_inference_steps}NFE_cam{cfg.eval.cam_type}.mp4", fps=fps)
