...
class TrainingModule(pl.LightningModule):
    def __init__(
        ...
        module_name_list = ['self_attn', 'cross_attn', 'time_embedding', 'time_embedding_r', 'time_projection', 'rel_pose_embedding']
        ...
        self.learning_rate = cfg.meanflow.lr
    ...
    def forward_preprocess(self, data):
        ...
        inputs_shared = {
            ...
            "distill": True,                                        # Stage 2: always take the distillation branch
    ...
    def on_train_epoch_end(self):
        if self.trainer.is_global_zero:
            ...
            save_file(state_dict, os.path.join(checkpoint_dir, f"meanflow_epoch_{self.trainer.current_epoch + 1}.safetensors"))
...
def main(cfg: DictConfig):
    ...
    if not cfg.wandb.disabled:
        ...
        wandb_logger = WandbLogger(
            ...
            name = f"meanflow-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
    ...
    if cfg.meanflow.get("resume_checkpoint", None):
        ckpt_path = cfg.meanflow.resume_checkpoint
        state_dict = load_file(ckpt_path)
        missing, unexpected = model.pipe.dit.load_state_dict(state_dict, strict=False)
        print(f"Initialized from Stage 1 checkpoint {ckpt_path}, loaded {len(state_dict)} params, missing={len(missing)}, unexpected={len(unexpected)}")

    trainer = pl.Trainer(
        max_epochs=cfg.meanflow.max_epochs,
        accelerator="gpu",
        devices='auto',
